...
The team is adopting Vite+ as its unified toolchain. The current docs build can omit the library's latest output, which leaves stale components. Using `pnpm --filter` to include the library transitively has produced different results across pnpm versions. Dev mode also starts before the library's `dist/` exists and fails with missing modules.
...
- `package.json`: root manifest with a corrected `scripts` block; the rest may remain as provided
- `apps/docs/vite.config.ts`: Vite config for the docs app
- `vite.config.ts`: root Vite config with workspace-wide hook and staged configuration
- `packages/luminary-ui/package.json`: library manifest with corrected scripts
...
=============== FILE: packages/luminary-ui/package.json ===============
{
  "name": "@luminary/ui",
  "version": "0.5.0",
  "description": "UI component library for the Luminary Design System",
  "type": "module",
  "exports": {
    ".": {
