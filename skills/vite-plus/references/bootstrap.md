...
2. Install the current lockfile, then use `pnpm exec vp migrate` as the default starting point instead of a hand-rolled conversion. Pass `--agent <name>` and `--editor <name>` to write agent and editor config in the same pass; pass `--no-interactive` for non-interactive runs. If the repo does not yet depend on Vite+, use `pnpm dlx vite-plus migrate` once, then switch to the installed CLI.
...
8. For existing Vite+ projects, current `pnpm exec vp migrate` performs a version-only upgrade by default: it re-pins `vite-plus`, the `vite` -> `@voidzero-dev/vite-plus-core` alias, and Vitest-related pins across workspace packages without re-running first-time setup. Use `--full` only when you intentionally want hooks, editor files, agent files, and lint migration touched again.
