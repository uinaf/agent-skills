...
7. Treat the machine-global `vp` binary and the repo-local `vite-plus` package as separate upgrade surfaces. Use `vp upgrade` for an official-installer CLI; when a package manager or tool manager owns `vp` and omits that command, upgrade it through that manager instead. Project dependencies should normally move with `vp migrate` from the workspace root.
