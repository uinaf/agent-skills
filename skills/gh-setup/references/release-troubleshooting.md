...
- Fix: `actions/checkout@<full-sha> # v7.0.1` with `fetch-depth: 0` on **both** verification and release jobs.
...
- Use the npm CLI to register or repair the trusted publisher: `npx -y npm@^11.15.0 trust github <package-name> --repo <owner>/<repo> --file <workflow-file> --env <environment> --allow-publish --yes`.
