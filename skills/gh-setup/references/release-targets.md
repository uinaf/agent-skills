...
- Set commit author/committer to `<app-slug>[bot]` / `{user-id}+<app-slug>[bot]@users.noreply.github.com`, resolving `{user-id}` at runtime from `/users/{app-slug}[bot]` (see Bot Identity in `release-workflows.md`).
