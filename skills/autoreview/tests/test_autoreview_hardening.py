...
class AutoreviewHardeningTests(unittest.TestCase):
    ...
    def test_local_models_and_claude_effort_defaults(self) -> None:
        ...
        self.assertEqual(codex.model_candidates, ["gpt-5.6-sol"])
        self.assertEqual(
            claude.model_candidates,
            ["claude-opus-5"],
        )
        self.assertEqual(claude.model, "claude-opus-5")
        self.assertEqual(claude.thinking, "high")
        self.assertIsNone(claude.claude_preferred_model_tail)
    ...
    def test_claude_runs_outside_repo_with_auto_memory_disabled(self) -> None:
        args = argparse.Namespace(
            ...
            claude_bin="claude",
            claude_preferred_model_tail=None,
            model="claude-opus-5",
            stream_engine_output=False,
            thinking="high",
            tools=False,
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertIn(
                ["--model", "claude-opus-5"],
                [observed["cmd"][index:index + 2] for index in range(len(observed["cmd"]) - 1)],
            )
            self.assertIn(
                ["--effort", "high"],
                [observed["cmd"][index:index + 2] for index in range(len(observed["cmd"]) - 1)],
