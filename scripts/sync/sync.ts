...
function ruleTargets(repoDir: string): RuleTargets {
  return {
    agents: join(repoDir, "rules", "agents.final.md"),
    claude: join(repoDir, "rules", "claude.final.md"),
  };
}

function generateRules(repoDir: string, targets: RuleTargets): void {
  const baseRules = join(repoDir, "rules", "agents.md");
  const claudeRules = join(repoDir, "rules", "claude.md");
  ...
  const claudeHeader =
    "# Agent Instructions\n\n" +
    "Generated by scripts/sync/sync.sh from rules/agents.md, rules/claude.md, and optional rules/agents.local.md. Do not edit directly.\n\n" +
    "---\n\n";
  const localOverrides = existsSync(localRules)
    ? `\n---\n\n## Local Overrides\n\n${readFileSync(localRules, "utf8")}`
    : "";
  const base = readFileSync(baseRules, "utf8");
  const claude = readFileSync(claudeRules, "utf8");
  const temporaryDirectory = mkdtempSync(join(dirname(targets.agents), ".agents-final-"));
  const temporaryAgentsRules = join(temporaryDirectory, "agents.final.md");
  const temporaryClaudeRules = join(temporaryDirectory, "claude.final.md");

  try {
    writeFileSync(temporaryAgentsRules, `${agentsHeader}${base}${localOverrides}`);
    writeFileSync(
      temporaryClaudeRules,
      `${claudeHeader}${base}\n---\n\n${claude}${localOverrides}`,
    );
    renameSync(temporaryAgentsRules, targets.agents);
    renameSync(temporaryClaudeRules, targets.claude);
  } finally {
    rmSync(temporaryDirectory, { force: true, recursive: true });
  }
}
...
function findAgentLinks(runtime: Runtime, home: string, targets: RuleTargets): AgentLink[] {
  ...
  if (runtime.commandExists("claude")) {
    links.push({
      ...
      managedTargets: [targets.claude, targets.agents],
...
function unmanagedLinkReason(destination: string, managedTargets: readonly string[]): string | undefined {
  try {
    ...
    const resolvedTarget = resolve(dirname(destination), currentTarget);
    if (!managedTargets.some((target) => resolvedTarget === resolve(target))) {
...
function preflightAgentLinks(links: readonly AgentLink[]): void {
  const conflicts = links.flatMap((link) => {
    const reason = unmanagedLinkReason(link.destination, link.managedTargets);
    return reason === undefined ? [] : [`${link.destination}: ${reason}; expected ${link.target}`];
  });
  ...
  if (conflicts.length > 0) {
    throw new Error(
      "Refusing to replace unmanaged global agent rules:\n" +
        ...
        "\nMove the listed paths aside or link each destination to its expected target, then rerun sync.",
    );
  }
}

function configureAgents(runtime: Runtime, links: readonly AgentLink[]): Agent[] {
  for (const link of links) {
    replaceSymlinkAtomically(link.target, link.destination);
    writeLine(runtime.stdout, `Linked: ${link.label} -> rules/${basename(link.target)}`);
  }
...
function sync(runtime: Runtime): number {
  ...
  const targets = ruleTargets(repoDir);
  const links = findAgentLinks(runtime, home, targets);
  preflightAgentLinks(links);

  generateRules(repoDir, targets);
  writeLine(runtime.stdout, "Generated: rules/agents.final.md rules/claude.final.md");
  const agents = configureAgents(runtime, links);
