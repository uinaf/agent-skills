...
function claudeFinalRulesPath(repoDir: string): string {
  return join(repoDir, "rules", "claude.final.md");
}

function createManagedRuleLinks(repoDir: string, home: string, legacyClaudeLink = false): void {
  mkdirSync(join(home, ".claude"), { recursive: true });
  mkdirSync(join(home, ".codex"), { recursive: true });
  symlinkSync(
    legacyClaudeLink ? finalRulesPath(repoDir) : claudeFinalRulesPath(repoDir),
    join(home, ".claude", "CLAUDE.md"),
  );
  symlinkSync(finalRulesPath(repoDir), join(home, ".codex", "AGENTS.md"));
}
...
test("completes a successful sync and preserves rules links", () => {
  ...
  assert.equal(
    readFileSync(join(repoDir, "rules", "claude.final.md"), "utf8"),
    "# Agent Instructions\n\n" +
  ...
  assert.equal(
    readlinkSync(join(home, ".claude", "CLAUDE.md")),
    claudeFinalRulesPath(repoDir),
  );
...
test("includes ignored local overrides in generated rules", () => {
  ...
  assert.match(
    readFileSync(claudeFinalRulesPath(repoDir), "utf8"),
    /# Claude, you should pay extra attention to these[\s\S]*## Local Overrides\n\n### Private machine override/,
  );
...
test("replaces links already managed by this sync target", () => {
  ...
  writeFileSync(claudeFinalRulesPath(repoDir), "old generated Claude rules\n");
  createManagedRuleLinks(repoDir, home, true);
  ...
  assert.match(
    readFileSync(claudeFinalRulesPath(repoDir), "utf8"),
    /# Updated fixture agent rules[\s\S]*# Claude, you should pay extra attention to these/,
  );
  assert.equal(
    readlinkSync(join(home, ".claude", "CLAUDE.md")),
    claudeFinalRulesPath(repoDir),
...
test("refuses every unmanaged global destination without partial mutation", () => {
  ...
  assert.equal(existsSync(claudeFinalRulesPath(repoDir)), false);
